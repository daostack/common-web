import type {
  ...
  OrderBy,
  FieldFilter,
} from "@firebase/firestore/dist/firestore/src/core/target";
import type {
  Value,
  ArrayValue,
  MapValue,
...
export function stringifyQuery(_query: any) {
  const query = _query._delegate;
  if ("_query" in query) {
...
function stringifyTarget(target: Target): string {
  ...
  if (target.orderBy.length > 0) {
    const order = target.orderBy.map((o) => stringifyOrderBy(o)).join(", ");
    str += `|ob:[${order}]`;
  }
  if (target.startAt) {
    str += "|lb:";
...
export function stringifyFilter(filter: FieldFilter): string {
  return `${filter.field.canonicalString()} ${filter.op} ${canonifyValue(
    filter.value,
  )}`;
}
...
function canonifyMap(mapValue: MapValue): string {
  ...
  const content = sortedKeys
    .map((key) => `${key}:${canonifyValue(mapValue.fields![key])}`)
    .join(",");
  return `{${content}}`;
}
