...
const DiscussionMessageContainer = () => {
  const { id: discussionMessageId } =
    useParams<DiscussionMessageRouterParams>();
  const [currentDiscussionMsg, setCurrentDiscussionMsg] =
    useState<DiscussionMessage | null>(null);
  const [discussionWithHighlightedMsg, setDiscussionWithHighlightedMsg] =
    useState<DiscussionWithHighlightedMessage | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const requestingDiscussionMsg = await fetchDiscussionMessageById(
          discussionMessageId,
        );
        const relatedDiscussion = await fetchDiscussionById(
          requestingDiscussionMsg.discussionId,
        );
  ...
  useEffect(() => {
    if (!currentDiscussionMsg || !discussionWithHighlightedMsg) return;
