...
import { Common, Card, Currency } from "@/shared/models";
...
export default function RequestPayment(
  ...
  const [{ payment, isPaymentLoading, isPaymentIframeLoaded }, setState] =
    useState<State>(INITIAL_STATE);
  const [cards, setUserCards] = useState<Card[]>([]);
  const [hasPaymentMethod, setHasPaymentMethod] = useState<boolean | null>(
    null,
  );
  const [isImmediatePaymentLoading, setIsImmediatePaymentLoading] =
  ...
  const makeImmediateContributionRequest = useCallback(() => {
    if (!paymentData.contributionAmount) return;
  ...
  useEffect(() => {
    if (!isPaymentIframeLoaded || !payment || !newCardId) return;

    try {
      return subscribeToCardChange(newCardId, (card) => {
        if (card) makeImmediateContributionRequest();
      });
    } catch (error) {
      onError(
        (error as Error).message ||
          "Error during subscription to payment status change",
      );
    }
  }, [
