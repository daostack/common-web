/*
  ...
  - The migration will remove the values [Confirmed,Failed] on the enum `ProposalPaymentState`. If these variants are still used in the database, the migration will fail.

*/
-- AlterEnum
BEGIN;
CREATE TYPE "ProposalPaymentState_new" AS ENUM ('NotAttempted', 'Pending', 'Successful', 'Unsuccessful');
